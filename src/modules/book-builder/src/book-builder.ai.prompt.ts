import { IBookBuilderChapter, IBookBuilderGeneration } from "./book-builder.interface";

export const BOOK_OUTLINE_PROMPT = {
...
Do not add unknown explanation, greeting, or extra text outside the JSON.`,
  user: (selectedAuthor: string, selectedSkills: string) => `Create the book outline now using the author "${selectedAuthor}" and the desired skills "${selectedSkills}".`,
};

export const CHAPTER_WRITE_PROMPT = {
  system: (selectedAuthor: string, selectedSkills: string, outline: IBookBuilderGeneration, allChapters: IBookBuilderChapter[], chapterNumber: number) => {
    const currentChapter = allChapters.find((c) => c.number === chapterNumber);
    return `You are now writing the actual book as ${selectedAuthor}.
...
Write the COMPLETE chapter content only. Do not output JSON, do not say "Chapter X", do not add unknown meta text. Start directly with the first sentence of the chapter.`;
  ...
  user: (chapterNumber: number) => `Begin writing Chapter ${chapterNumber} now.`,
};