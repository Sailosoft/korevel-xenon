...
const THEME = {
  ...
  navHover: "text-slate-600 hover:bg-slate-50 hover:text-[#ff2d20] transition-colors",
...
function BunnyAILayoutContent({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`h-screen flex overflow-hidden ${isOpen ? "sidebar-open" : ""} ${THEME.bgWindow}`}>
        ...
        <header className="glass-header h-16 flex items-center justify-between px-4 md:px-8 flex-shrink-0 z-10 sticky top-0">
          <div className="flex items-center space-x-4">
            ...
            <div className="p-6 flex items-center space-x-3">
              ...
              <span className={`text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${THEME.gradient}`}>
...
const Sidebar = ({
  ...
  return (
    <aside
      id="sidebar"
      className={`fixed inset-y-0 left-0 z-30 ${THEME.bgSidebar} ${THEME.border} flex flex-col sidebar-transition md:relative overflow-hidden ${isOpen
    ...
    >
      <div className="w-72 flex flex-col h-full flex-shrink-0">
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
          <button className={`w-full flex items-center space-x-3 px-4 py-3 rounded-2xl transition-all shadow-md ${THEME.btnPrimary} ${THEME.shadow} mb-6 group cursor-pointer`}>
            <Plus />
            <span className="font-medium">New Project</span>
          </button>

          <div className={`text-xs font-semibold uppercase tracking-wider px-4 mb-2 ${THEME.textMuted}`}>
            Main Menu
          </div>

          <Link
            href="/modules/bunny-ai"
            className={`flex items-center space-x-3 px-4 py-2.5 rounded-xl transition-colors group ${pathname === "/modules/bunny-ai" ? THEME.navActive : THEME.navHover
              }`}
          >
          ...
          <Link
            ...
            className={`flex items-center space-x-3 px-4 py-2.5 rounded-xl transition-colors group ${pathname === "/modules/bunny-ai/authors" ? THEME.navActive : THEME.navHover
              }`}
          >
          ...
          <div className={`text-xs font-semibold uppercase tracking-wider px-4 mt-8 mb-2 ${THEME.textMuted}`}>
            Settings
          </div>
          <Link
            href="#"
        ...
        <div className={`p-4 border-t ${THEME.border}`}>
          <div className="bg-gray-50 rounded-2xl p-3 flex items-center space-x-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold ${THEME.avatarBg} ${THEME.avatarText}`}>
              BA
            </div>
